  ...
  return (
    <div className="max-w-[1500px]">
      <nav className="flex items-center justify-between bg-linear-to-r from-[#54AE7A] to-[#299574] px-4 py-2 text-white">
        ...
        <ul className="md:flex items-center justify-between hidden gap-6 text-slate-900">
          {links.map((link, index) => (
            <li
              key={index}
              className="cursor-pointer  rounded-full  font-normal px-6 py-2 text-white "
            >
              <NavLink
                to={link.path}
                className={({ isActive }) =>
                  isActive
                    ? "text-[#EB744A] font-bold"
